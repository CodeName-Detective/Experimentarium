...
def test_trainer_runs_one_epoch(tmp_path, tiny_cfg):
    ...
    trainer = Trainer(tiny_cfg, model, task, loaders, optimizer, scheduler, LoggerCollection([NoopLogger()]), CheckpointManager(tmp_path))
    metrics = trainer.fit()
...
def test_trainer_resume_at_max_epoch_runs_no_new_epochs(tmp_path, tiny_cfg):
    ...
    resumed = Trainer(resume_cfg, reloaded, task, loaders, optimizer, scheduler, LoggerCollection([NoopLogger()]), manager)
    resumed.fit()
