...
def latest_record_for_run(records: list[dict[str, Any]], run_id: str) -> dict[str, Any]:
...
def config_path_for_run(run_id: str, registry_path: str | Path = DEFAULT_REGISTRY_PATH) -> Path:
    ...
    if records:
        try:
            ...
            pass
    fallback = Path('outputs/run_configs') / f'{run_id}.yaml'
    if fallback.exists():
        return fallback
...
def training_run_dir_for_record(record: dict[str, Any]) -> Path:
    """Return the training run directory associated with a registry record."""
    run_id = str(record.get('run_id', ''))
...
def run_status(record: dict[str, Any]) -> str:
    ...
    if any(checkpoint_dir.glob('*_exception.pt')):
        return 'failed'
    if log_path.exists():
    ...
    if mode in {'eval', 'test'}:
        return 'success' if metrics_path.exists() else 'incomplete'
    if mode == 'profile':
    ...
    if manifest.exists() and metrics_path.exists():
        return 'success'
    return 'incomplete'
