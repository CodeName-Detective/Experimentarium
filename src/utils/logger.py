...
class LoggerBackend(Protocol):
    ...
    def log_artifact(self, path: str | Path, name: str, artifact_type: str = 'artifact', metadata: dict[str, Any] | None = None) -> None: ...
...
class ConsoleLogger:
    ...
    def log_metrics(self, metrics: dict[str, float], step: int | None = None) -> None:
        ...
        self.logger.info(prefix + ' | '.join(f'{key}={value:.5f}' for key, value in metrics.items()))
...
class JsonlLogger:
    ...
    def log_artifact(self, path: str | Path, name: str, artifact_type: str = 'artifact', metadata: dict[str, Any] | None = None) -> None:
        if self.handle is None:
            return
        self.handle.write(json.dumps({'artifact': {'path': str(path), 'name': name, 'type': artifact_type, 'metadata': metadata or {}}}, sort_keys=True) + '\n')
        self.handle.flush()
        ...
        try:
            ...
            if config_path is not None and Path(str(config_path)).exists():
                artifact = wandb.Artifact(
                    ...
                    type='config',
                    metadata={'run_id': cfg_get(cfg, 'run.id', None), 'config_id': cfg_get(cfg, 'run.config_id', None)},
                )
...
def setup_python_logging(cfg) -> None:
...
def build_loggers(cfg) -> LoggerCollection:
...
def init_wandb(cfg) -> None:
    """Compatibility helper that now builds all configured loggers."""
